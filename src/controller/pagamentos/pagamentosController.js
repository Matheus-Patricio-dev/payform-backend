...
const createPayment = async (req, res) => {
    try {
        ...
        if (payment) {
            ...
            if (!seller) {
                return res.status(404).json({ message: "Seller não encontrado para o cliente_id informado." });
            }
            ...
            if (
                ...
                payment.paymentMethods.includes('pix')
            ) {
                const zoopCreateTransaction = await generateTransaction(marketplaceId, sellerId, payment);
            }
        }
