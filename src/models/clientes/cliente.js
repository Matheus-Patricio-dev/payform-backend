...
class Cliente {
  static async buscarPorEmail(email) {
    ...
    const marketplaceQuery = await db
    ...
    if (clienteData.id_juros) {
      const jurosDoc = await db
        .collection("juros")
        .doc(clienteData.id_juros)
        .get();
      if (jurosDoc.exists) {
        const jurosData = jurosDoc.data();
        taxa_juros = jurosData.amount;
        juros_data = jurosData;
      }
    }
    ...
    return {
      ...
      ...(juros_data !== null && { juros_data }), // só adiciona se existir
    };
  }
  ...
  static async buscarSellersPorMarketplace(marketplaceId) {
    ...
    const querySnapshot = await sellersRef
    ...
    const clientesPromises = sellers.map(async (seller) => {
      const clienteSnap = await db
        .collection("clientes")
        .doc(seller.cliente_id)
        .get();

      return {
        ...seller,
        cliente: clienteSnap.exists
          ? { id: clienteSnap.id, ...clienteSnap.data() }
          : null,
      };
    });
  ...
  static async buscarRelacionamentoCompletoPorClienteId(id) {
    ...
    try {
      ...
      const clienteSeller = {
        id: clienteSellerDoc.id,
        ...clienteSellerDoc.data(),
      };
      ...
      if (
        ...
        typeof seller.marketplaceId !== "string" ||
        seller.marketplaceId.trim() === ""
      ) {
      ...
      if (
        marketplace.cliente_id &&
        typeof marketplace.cliente_id === "string" &&
        marketplace.cliente_id.trim() !== ""
      ) {
        const clienteMarketplaceDoc = await db
          .collection("clientes")
          .doc(marketplace.cliente_id)
          .get();
        if (clienteMarketplaceDoc.exists) {
          clienteMarketplace = {
            id: clienteMarketplaceDoc.id,
            ...clienteMarketplaceDoc.data(),
          };
        }
      }
      ...
      throw new Error(
        "Erro ao buscar relacionamento completo: " + error.message
      );
    }
  }
  ...
  static async buscarPorIdMKTPayment(id) {
    try {
      ...
      throw new Error(
        "Erro ao buscar cliente pelo marketplace: " + error.message
      );
    }
  }
    ...
    const payments = await Promise.all(
      paymentsSnapshot.docs.map(async (paymentDoc) => {
        ...
        const transactionsSnapshot = await transactionsRef
          .where("pagamento_id", "==", paymentDoc.id)
          .get();
