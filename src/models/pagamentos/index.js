...
class Payment {
  ...
  static async getById(id) {
    ...
    // Verifica se o pagamento tem id_juros
    if (paymentData?.id_juros) {
      const jurosDoc = await db
        .collection("juros")
        .doc(paymentData?.id_juros)
        .get();
      if (jurosDoc.exists) {
        paymentData.juros = { id: jurosDoc.id, ...jurosDoc.data() };
      }
    }
    ...
    const transacoes = await Promise.all(
      transactionsSnapshot.docs.map(async (doc) => {
        ...
        const paymentDoc = await db
          .collection("pagamentos")
          .doc(transactionData.pagamento_id)
          .get();
        if (paymentDoc.exists) {
          transactionData.pagamento = {
            id: paymentDoc.id,
            ...paymentDoc.data(),
          };
        } else {
  ...
  static async getByUserRoleTransacoes({
    userId,
    cargo,
    marketplaceId = null,
  }) {
    ...
    const transacoes = await Promise.all(
      transactionsSnapshot.docs.map(async (doc) => {
        ...
        const paymentDoc = await db
          ...
          .doc(transactionData.pagamento_id)
          .get();
        transactionData.pagamento = paymentDoc.exists
        ...
        const sellerDoc = await db
          .collection("clientes")
          .doc(transactionData.seller_id)
          .get();
        transactionData.cliente = sellerDoc.exists
          ? { id: sellerDoc.id, ...sellerDoc.data() }
          : null;
  ...
  static async delete(id) {
    ...
    const hasCompletedTransaction = transactionsSnapshot.docs.some(
      (doc) => doc.data().status === "completa"
    );
